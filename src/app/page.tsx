  ...
  return <div ref={pageRef}>
    ...
    <main id="top" className={ready?"site-ready":""}>
      ...
      <section id="ecosystem" className="ecosystem section">
        ...
        <div className="feature-grid">{features.map((f)=><a className="feature" href={f.n==="01"?"/titan-hardware-manager":f.n==="02"?"/auto-gpu-switcher":f.n==="04"?"/titanshare":f.n==="05"?"/titanmirror":"#ecosystem"} key={f.n}><div className="feature-top"><span>{f.n}</span><em>{f.tag}</em></div><h3>{f.title}</h3><p>{f.text}</p><code>{f.meta}</code></a>)}</div>
      </section>
